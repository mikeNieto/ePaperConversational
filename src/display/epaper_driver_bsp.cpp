...
epaper_driver_display::epaper_driver_display(int width, int height, custom_lcd_spi_t _lcd_spi_data) :
    ...
    if (!buffer) {
        ESP_LOGE(TAG, "FATAL: Cannot allocate %d bytes for display buffer", lcd_spi_data.buffer_len);
        abort();
    }
    ESP_LOGI(TAG, "Display buffer allocated: %p (%d bytes)", buffer, lcd_spi_data.buffer_len);
}
